...
public class ColorMappingColorChooserTest {
	...
	public void testCreateIterSpectrumMap() {
		...
		// # of entries in each Map must be the same
		assertTrue(testIterSpectrumMap.size() == iterCountMap.size());
		...
		// test for equality of all occurrences for those keys
		for (Integer key : testKeySet) {
			System.out.println("value: " + testIterSpectrumMap.get(key) + "  " + iterCountMap.get(key));
			assertTrue((int)testIterSpectrumMap.get(key) == (int)iterCountMap.get(key));
		}		
	...
	public void testCreateIterColorMap() {
		...
		// test for equality of all occurrences for those keys
		for (Integer key : testKeySet) {
			System.out.println("value: " + testIterColorMap.get(key) + "  " + iterColorMap.get(key));
			assertTrue(testIterColorMap.get(key) == iterColorMap.get(key));
		}		
	}
